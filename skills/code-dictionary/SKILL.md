...
모든 서브프로젝트 개발 시 에이전트는 중복 코드를 무분별하게 양산하거나 반대로 불필요한 추상화를 조기에 도입(Premature Abstraction)하지 않도록 아래의 업계 표준 **추출(Extraction) 및 승격(Promotion) 기준**을 반드시 준수해야 합니다.

---

### ① 🖥️ 프론트엔드 (Frontend - React) 진화 규칙

프론트엔드에서는 **"WET(Write Everything Twice)보다 섣부른 추상화 방지"**, **"인지적 과부하 최소화"**, **"엄격한 결합도 제어"**를 핵심 가치로 삼아 컴포넌트와 로직을 진화시킵니다.

1. **인라인 구현 (Inline Stage)**:
   - 새로운 피처 작성 시에는 빠른 흐름 파악과 유연성을 위해 하나의 컴포넌트 파일 안에 서브 요소를 인라인(Inline) 함수나 로컬 상태로 먼저 구현합니다.
2. **피처 내 컴포넌트 추출 (Extract to Local Feature Component)**:
   - **트리거 (WET 및 인지 부하)**:
     - 단일 컴포넌트 파일의 총 라인 수가 **150라인을 초과**하거나, JSX 내 중첩 깊이(Indentation)가 **5단계 이상** 깊어질 때.
     - 컴포넌트 내 `map` 루프 내부 구조가 10라인 이상으로 복잡해질 때 (아이템 컴포넌트로 추출).
     - UI 렌더링 코드와 복잡한 상태/이벤트 핸들러 로직이 섞여 한 눈에 코드 흐름을 파악하기 어려울 때.
   - **조치**: 동일 피처 폴더 내부(`features/{feature}/components/`)에 로컬 컴포넌트로 분리합니다.
3. **공통 컴포넌트 승격 (Promote to Shared/UI Component)**:
   - **트리거 (Rule of Three)**:
     - **3개 이상의 독립된 피처**(`features/A`, `features/B`, `features/C`)에서 동일한 UI 패턴의 공유가 요구될 때.
     - 단, 버튼/인풋 등 극도로 범용적인 아토믹 UI 요소는 **2개 피처**에서 사용되는 즉시 승격할 수 있습니다.
   - **조치**: 프로젝트의 전역 공통 폴더(`src/components/ui/` 또는 `src/components/shared/`)로 이관합니다.
   - **🛑 필수 제약조건 (의존성 디커플링)**:
     - 승격된 공통 컴포넌트는 **특정 피처의 전역 상태(Zustand), 데이터 페칭 쿼리(React Query), 혹은 라우터 컨텍스트(useParams 등)를 내부에서 직접 참조해서는 안 됩니다**.
     - 모든 제어 및 데이터 전달은 오직 **Props**를 통해서만 주입받도록 인터페이스를 재설계(無상태화)해야 합니다.
4. **로직과 UI의 격리 (Extract to Custom Hook/Util)**:
   - **트리거 (단일 책임 원칙 - SRP)**:
     - 컴포넌트 내부에서 비동기 API 통신, 타이머 제어, 혹은 다단계 상태 전이 등 순수 UI 렌더링과 관련 없는 **비즈니스 흐름 제어가 15줄을 초과**할 때.
   - **조치**: 비즈니스 로직만 커스텀 훅(`features/{feature}/hooks/`)으로 추출하고, 컴포넌트는 오직 훅의 상태와 함수만을 받아 렌더링하도록 격리시킵니다.

---

### ② ⚙️ 백엔드 (Backend - FastAPI) 진화 규칙

백엔드에서는 **"Thin Controller(얇은 컨트롤러)"** 지향과 **"비즈니스 결합도 제어"**를 통해 변경 용이성을 확보합니다.

1. **라우터 인라인 구현 (Inline Stage)**:
   - 단순한 단일 엔티티 조회, 혹은 간단한 CRUD와 같이 중간 비즈니스 가공 공식이 없는 1단계 수준의 DB 연산은 서비스 클래스 없이 라우터(`router.py`) 함수 내부에 인라인 코드로 작성할 수 있습니다.
2. **도메인 서비스 추출 (Extract to Domain Service/Usecase)**:
   - **트리거**:
     - 라우터 내 비즈니스 로직 코드가 **5줄을 초과**하거나, **2단계 이상의 데이터베이스 트랜잭션**을 묶어서 실행해야 할 때.
     - 여러 API 엔드포인트(혹은 CLI 명령, 크론 배치)에서 동일한 비즈니스 유효성 검사 및 정산 공식 등을 공유해야 할 때.
   - **조치**: 도메인 서비스 클래스(`service.py` 또는 `usecase/` 레이어)로 로직을 들어내어 비즈니스 경계를 세웁니다.
3. **공통 인프라/유틸 승격 (Promote to Shared Clients & Dependencies)**:
   - **트리거**: 2개 이상의 독립된 도메인 영역에서 동일한 인프라성 기능(메일 발송, 파일 업로드 SDK, 외부 API 연동 등)이나 전역 공통 보안 필터(Depends)가 필요할 때.
   - **조치**: `shared/clients/` 또는 `shared/dependencies.py` 등의 전역 공유 폴더로 승격합니다.
   - **🛑 필수 제약조건 (비즈니스 결합 금지)**:
     - 공통 라이브러리/공유 폴더에는 **특정 도메인의 비즈니스 로직을 포함시켜 공유하지 않습니다**. 비즈니스 규칙을 공유 모듈로 엮기 시작하면 서비스 간의 강한 결합(Tight Coupling)을 유발해 버전 업 시 전체 서비스가 깨지는 배포 지옥이 발생합니다.
     - 도메인 간의 의존이 필요하다면 공통 코드로 합치지 말고 **API 엔드포인트 호출**이나 **메시지 큐**를 통해 통신하도록 설계합니다.

---

### ③ 🤖 AI 서버 (AI Module) 진화 규칙

AI 추론기 및 에이전트 서비스에서는 **"단위 테스트의 격리성(Testability without Weights)"**을 확보하기 위해 물리적 레이어를 진화시킵니다.

1. **단일 스크립트 구현 (Inline Stage)**:
   - 신규 모델의 초기 가중치 테스트 및 간단한 배치 분석은 `main.py`나 일회성 분석 스크립트에 데이터 전처리, 모델 추론(Predict), 결과를 저장하는 후처리를 한 번에 인라인으로 구현하여 검증합니다.
2. **핵심 연산 및 추론 격리 (Extract to Preprocessor/Adapter)**:
   - **트리거 (가중치 로딩 의존성 제거)**:
     - 입력 데이터(텍스트, 신호, 이미지)의 전처리 알고리즘과 ONNX/PyTorch 등의 런타임 세션 구동 코드가 섞여 있어, **무거운 AI 모델 가중치를 로드하지 않고는 단순 전처리 로직만 단위 테스트(Unit Test)하는 것이 불가능**할 때.
   - **조치**: 전처리/후처리 비즈니스 로직은 순수 Python CPU 연산 위주의 `processor.py`(`[AI-DOMAIN-CORE]`)로 추출하고, 무거운 가중치를 다루는 구동부는 `adapter.py`(`[AI-DOMAIN-ADAPTER]`)로 레이어를 분리합니다.
3. **공통 수학/신호처리 유틸 승격 (Promote to AI Shared Util)**:
   - **트리거**: 여러 모델 파이프라인에서 동일한 수학적 정규화(Normalization), 데이터 보간(Interpolation) 연산, 또는 특수 피처 추출 공식을 중복 적용해야 할 때.
   - **조치**: `shared/utils/` 하위로 이동시켜 도메인 지식이 없는 순수 연산 유틸리티 모듈로 승격시켜 공유합니다.
