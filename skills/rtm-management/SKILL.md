...
## 2. 설계 및 작성 원칙
1. **아키텍처별 매핑 테이블 분리 및 코드폼 명칭 단위 컬럼 매핑**: 시나리오별 기술 매핑은 프론트엔드(FE), 백엔드(BE), AI 모듈의 아키텍처별로 테이블을 분리하여 기재합니다. 각 테이블의 컬럼은 표준 아키텍처 규격을 명시하는 **코드폼 명칭(FE-PAGE, BE-ROUTER 등)**과 기타 유형을 위한 **etc**로 이루어진 총 9개의 컬럼으로 구성됩니다.
    ...
    - **코드폼 명칭(Code Form Name) 매핑**: 테이블 셀 내의 소스 코드 파일 경로는 마크다운 링크 형식을 사용하지 않고 텍스트(파일명)로 표기하며, **해당 파일이 어떤 코드폼 유형에 속하는지에 따라 알맞은 코드폼 컬럼(예: FE-PAGE, BE-ROUTER)**에 배치합니다. 이때, 필수 컬럼에 들어가는 파일명 뒤에는 대괄호 유형 접미사(예: `[FE-INPUT]`, `[BE-ROUTER]` 등)를 붙이지 않고 오직 순수 파일명만 기입합니다.
    ...
    - **프론트엔드(Frontend) 매핑 테이블 컬럼**:
      - `FE-PAGE`, `FE-FEATURE`, `FE-INPUT`, `FE-QUERY`, `FE-MUTATION`, `FE-STORE`, `FE-HOOK`, `FE-UTIL`, `etc` 순으로 정렬합니다. (App Provider, App Router, Form Container Wrapper 등 프로젝트 전역/공통 컴포넌트는 매핑 테이블 컬럼에서 제외합니다.)
    - **백엔드(Backend) 매핑 테이블 컬럼**:
      - `BE-ROUTER`, `BE-SERVICE`, `BE-VO`, `BE-MODEL`, `BE-DEPENDENCY`, `BE-SCHEMA`, `BE-TEST`, `BE-UTIL`, `etc` 순으로 정렬합니다. (Database Session Manager 등 프로젝트 전역/단일 생성 파일은 매핑 테이블 컬럼에서 제외합니다.)
    - **AI 모듈(AI Module) 매핑 테이블 컬럼**:
      - `AI-ROUTER`, `AI-USECASE`, `AI-WORKFLOW`, `AI-CORE`, `AI-GATEWAY`, `AI-TYPE`, `AI-TEST`, `AI-UTIL`, `etc` 순으로 정렬합니다. (Bootstrap & DI Container 등 프로젝트 전역/단일 생성 파일은 매핑 테이블 컬럼에서 제외합니다.)
