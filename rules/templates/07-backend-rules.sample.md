...
본 가이드는 FastAPI 기반 백엔드 애플리케이션의 높은 안정성과 일관성을 확보하기 위해 AI 에이전트가 반드시 준수해야 하는 규칙입니다. 코드의 세부 구현 양식(Boilerplate)은 규칙 파일이 아닌 **기술 매핑 문서(`rtm_{기능명}.md` 가이드)**의 표준 코드 양식을 참조하십시오.
...
*   **상세 구조 및 규칙**: 세부 디렉토리 구조와 Bounded Context 간 상호 참조 임포트 규칙은 프로젝트의 기본 골격을 유지하며, `rtm_*.md` 파일의 [BE 도메인 표준 양식]을 준수합니다.
...
*   **코드 양식**: `rtm_*.md` 가이드의 [BE 아키텍처 표준 양식]을 참고하십시오.
...
*   **코드 양식**: `rtm_*.md` 가이드의 [BE 도메인 표준 양식]을 참고하십시오.
...
*   **코드 양식**: `rtm_*.md` 가이드의 [BE API 표준 양식] 및 [BE 아키텍처 표준 양식]을 참고하십시오.
...
*   **코드 양식**: `rtm_*.md` 가이드의 [BE API 표준 양식]을 참고하십시오.
