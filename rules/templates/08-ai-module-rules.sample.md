...
## [1부] 에이전트 & 도메인 아키텍처 (Agent & Domain Architecture)
...
### 3. 데이터 검증 및 Value/Boundary 모델링 규칙
...
### 4. 워크플로우 설계 및 상태 관리 규칙 (Workflow & State Management)
에이전트 서버의 `workflow`는 단순한 비즈니스 유스케이스를 넘어 전체 문서 처리 프로세스의 단계 전이(State Transition)와 실행 컨텍스트(Context)를 관리하는 핵심 오케스트레이터입니다.

- **워크플로우의 역할 및 책임 범위**:
  - **흐름 제어**: 사용자 입력 및 요청 파라미터를 파악하여 기 정의된 시나리오 흐름 중 어떤 실행 흐름(Flow)을 실행할지 결정합니다.
  - **단계 관리**: 현재 실행 단계(Step)와 다음 진입 단계를 추적 및 유지합니다.
  - **Usecase 조립**: 비즈니스 로직에 필요한 여러 `usecases`를 올바른 논리적 실행 순서로 결합하고 조율합니다.
  - **대화 및 승인 라이프사이클 관리**: Human-in-the-loop 검증 단계에서 일시 중지(`pending`), 승인(`approve`), 반려(`reject`), 재개(`resume`) 상태를 안전하게 제어합니다.
- **Workflow vs Tool vs Usecase 경계 구분 규칙**:
  - **`tool` (Stateless Entry)**: 상태가 없는 일회성 실행기이며, 자체적으로 큰 문맥이나 중간 산출물 메모리를 보관하지 않고 단순히 workflow를 호출하는 진입점입니다.
  - **`usecase` (Single Task)**: "Source 저장", "Unit 파싱", "Bundle 분석", "이메일 초안 생성"과 같은 단일 목적의 비즈니스 작업을 완결성 있게 격리 수행합니다.
  - **`workflow` (Stateful Orchestrator)**: 여러 `usecase`를 시나리오대로 연결하고 실행 Context를 추적하며, "승인 지연 시 중지 후 pending 상태 기록", "승인 인입 시 동일 문맥 재개"와 같은 다중 유스케이스 조율 책임을 전담합니다.
- **메모리 보호를 위한 State 제한 규칙 (중요)**:
  - 워크플로우는 상태(`state`)를 갖지만, **대용량의 원천 데이터 또는 가공 데이터를 state 메모리에 직접 담는 것을 엄격히 금지**합니다.
  - *State에 담을 수 있는 요소*: 현재 flow 이름, 현재 step명, 인메모리 참조 키 목록(Artifact Ref list), 현재 활성화된 ref 키, human decision 정보, 가벼운 결과 요약 문자열 및 에러 상세.
  - *State 유입 금지 대상*: 전체 엑셀 워크북 바이너리, 대용량 OCR 원문 텍스트, 판다스 데이터프레임(DataFrame), 첨부파일 raw bytes 등. 이러한 대규모 리소스는 반드시 외부 파일 저장소 또는 데이터베이스(Repository/Store)에 저장하고, 워크플로우 state에는 오직 **해당 데이터에 접근할 수 있는 식별자(Artifact Ref Key)만 관리**하십시오.
- **프레임워크 비종속 설계 (LangGraph 격리)**:
  - 워크플로우의 실행 제어 도구로 **LangGraph** 등의 프레임워크 기술을 활용하더라도, 도메인의 실행 흐름 설계(`src/[app_name]/workflow/`)는 기술 사양에 종속되지 않게 도메인 중심으로 캡슐화되어야 합니다.
  - 워크플로우 테스트 작성 시에는 프레임워크 엔진의 버그 테스트가 아닌, **우리가 정의한 비즈니스 라우팅 규칙(route 선택), 단계 전이 안정성(state safety) 및 승인/재개 전이 논리**를 검증하는 단위 테스트(`tests/unit/workflow/`)에 집중하십시오.

---

## [2부] 외부 연동 및 물리 격리 규칙 (External Integration & Physical Isolation)

### 5. 외부 인프라 및 Outbound 연동 격리 규칙 (Outbound Isolation)
...
### 6. 문서 수정 의도(Edit Intent) 및 자동화 격리 규칙 (Edit Intent & Automation)
...
### 7. LLM 연동 및 에코시스템 격리 규칙 (LLM Outbound & Configuration)
...
## [3부] 로컬 ML 리소스 및 성능 최적화 (Local ML Resources & Performance Optimization)

### 8. AI 모델 생명주기 및 리소스 관리 (OOM 예방)
...
### 9. 동시성 제어 및 이벤트 루프 블로킹 방지 (성능 보장)
...
### 10. GPU 메모리 관리 및 연산 최적화 (PyTorch 중심)
...
## [4부] 테스트 및 개발 표준 (Testing & Development Standards)

### 11. 추상 인터페이스 배제 기반 테스팅 전략 (Testing Without Abstract Adapters)
...
### 12. 프롬프트 관리 및 품질 평가 (Evaluation - `evals/` 구축)
...
### 13. 패키지 및 의존성 관리 표준 (`uv` & `pyproject.toml`)
...
### 14. 문서화 및 아키텍처 문서 가이드라인 (Documentation Standards)
