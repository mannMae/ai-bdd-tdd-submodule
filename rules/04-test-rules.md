...
```typescript
/**
 * @file {통합테스트파일명}.test.tsx (또는 .ts)
 * @description {시나리오명} 통합 테스트
 * 
...
const feature = loadFeature('./docs/user-flow/{시나리오명}.feature');
...
defineFeature(feature, (test) => {
  ...
  test('{시나리오 제목}', ({ given, when, then, and }) => {
    given('{시나리오 Given 조건}', () => {
      // Given 조건 검증 및 설정 코드
    });

    when('{시나리오 When 행동}', () => {
      // When 행동 수행 및 인터랙션 코드
    });
...
```typescript
/**
 * @file {유닛파일명}.test.ts (또는 .tsx)
 * @description {유닛명} 단위 테스트
 * 
 * [1. 테스트 대상 유닛 (SUT)]
 * - {유닛분류}: {파일경로} ({심볼명})
 *   - State: {상태필드 목록 및 기본값}
 *   - Actions: {호출가능한 액션/메서드 명세}
 * 
 * [2. 호출/의존하는 유닛 (Dependencies)]
 * - {의존유닛}: {파일경로} ({심볼명})
 * 
 * [3. SUT 동작 규칙 (Business Rules)]
 *   1. {규칙1}: {규칙설명} (요구처: {통합테스트파일명})
 *   2. {규칙2}: {규칙설명} (요구처: {통합테스트파일명})
 */
