...
- 코드를 변경하기 전에 항상 마스터 백로그(`docs/requirements/master_backlog.md`) 및 해당 기능의 기술 RTM 문서(예: `docs/requirements/technical_rtm_{기능명}.md`)를 먼저 확인하고 채점표를 작성/갱신하세요.

## 2. 업무 진행 단계 (요약)
모든 개발은 다음 순서에 따라 작업을 진행하며, 단계 중간에 AI 에이전트는 반드시 멈춰야 합니다.

1. **유저플로우 (User Flow) 및 시나리오 (Scenario) 작성**: 요구사항 확인 후 흐름 다이어그램(`*_flow.md`) 및 거킨 시나리오(`.feature`), 기술 RTM 문서(`technical_rtm_{기능명}.md`)를 작성합니다.
...
5. **자가 채점 및 커밋**: 완료 후 기술 RTM 문서의 자가 채점 진행 및 마스터 백로그의 Status를 업데이트하고 변경 사항을 논리적 단위로 커밋합니다.
