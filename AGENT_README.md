...
- **`skills/`**: Antigravity가 태스크 수행 시점에 자동으로 검색하여 동적으로 프롬프트에 주입하는 **실행형 스킬(Antigravity Skills)** 패키지입니다.
  - `bdd-workflow/`: BDD 프로세스 규칙 및 유저플로우/시나리오 설계 절차.
  - `tdd-development/`: Red-Green-Refactor 주기 준수 및 프로덕션 코드 구현 규칙.
  - `rtm-management/`: RTM 매핑 테이블 자가 채점 및 기록 규격.
  - `code-dictionary/`: 공통 기술 요소(API, Service 등) 표준 코드 폼(Code Form) 사전.
- **`templates/`**: 하위 프로젝트별 로컬 개발 가이드레일(`AI_GUIDERAILS`) 템플릿입니다. 프론트엔드, 백엔드(FastAPI), AI 모듈용 템플릿이 있습니다.
...
2. **기술 스택에 맞는 템플릿 복사**: 사용 중인 스택에 맞는 규칙 템플릿을 복사하여 배치합니다. (필요 없는 것은 삭제하거나 제외)
   ```bash
   # 프론트엔드 규칙 주입
   cp .agents/templates/06-frontend-rules.sample.md .agents-local/rules/06-frontend-rules.md
   
   # 백엔드(FastAPI) 규칙 주입
   cp .agents/templates/07-backend-rules.sample.md .agents-local/rules/07-backend-rules.md
   
   # AI 모듈 서버 규칙 주입
   cp .agents/templates/08-ai-module-rules.sample.md .agents-local/rules/08-ai-module-rules.md
   ```
